 ...
 * this is required also in character classes.) An identifier is a
 * string not containing right angle bracket (<tt>&gt;</tt>) or dash (<tt>-</tt>).
 * Numerical intervals are non-negative, decimal, and include both end
 * points, and if <tt><i>n</i></tt> and <tt><i>m</i></tt> have the same number of
 * digits, then the conforming strings must have that length (i.e. prefixed by 0's).
